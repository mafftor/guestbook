...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    #[ORM\OneToMany(mappedBy: 'author', targetEntity: Guestbook::class)]
    ...
    public function addGuestbook(Guestbook $guestbook): static
    {
        if (!$this->guestbooks->contains($guestbook)) {
            $this->guestbooks->add($guestbook);
            $guestbook->setAuthor($this);
        }

        return $this;
    }

    public function removeGuestbook(Guestbook $guestbook): static
    {
        if ($this->guestbooks->removeElement($guestbook)) {
            // set the owning side to null (unless already changed)
            if ($guestbook->getAuthor() === $this) {
                $guestbook->setAuthor(null);
            }
        }
